...
import {
    createWatchDomain,
    listWatchDomains,
} from "../../../services/watch-domain.service.js";

interface CreateWatchDomainBody {
    domain: string;
    label?: string;
...
export const handleCreateWatchDomain = async (
    request: FastifyRequest<{ Body: CreateWatchDomainBody }>,
    reply: FastifyReply
) => {
    try {
        ...
        const result = await createWatchDomain(
            user.tenantId,
            user.userId,
            request.body
        );
...
export const handleListWatchDomains = async (
