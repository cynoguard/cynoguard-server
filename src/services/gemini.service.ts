 ...
 */
export async function generateGeminiCandidates(
    watchDomain: string,
    count: number,
    ...
    try {
        ...
        for (const raw of candidates) {
            try {
                ...
                if (normalized === watchDomain) continue;
                if (existingCandidates.has(normalized)) continue;
                if (validCandidates.includes(normalized)) continue;
...
function buildPrompt(watchDomain: string, count: number): string {
