...
import type { CandidateGenerationResult } from "../types/domain-monitoring.js";
 ...
 * 4. Store in DB
 */
export async function generateAndStoreCandidates(
    ...
    const allCandidates: CandidateGenerationResult[] = [];
    ...
    if (remaining > 0) {
        const geminiDomains = await generateGeminiCandidates(
            watchDomain,
            remaining,
            seenDomains
        );
        ...
        for (const domain of geminiDomains) {
            ...
            if (allCandidates.length >= maxCount) break;
        }
    }
    ...
    for (const candidate of allCandidates) {
        try {
            ...
            console.error(
                `[CandidateGenerator] Error storing candidate ${candidate.domain}:`,
                error
            );
        }
    }
