 ...
 */
export function evaluateTldStrategy(candidateTld, watchTld, strategy, allowlist) {
    switch (strategy) {
        case "SAME_TLD_ONLY":
 ...
 */
export function computeSeverity(candidateTld, watchTld, tldSuspicious, similarityScore, threshold) {
    const criticalThreshold = Math.min(1.0, threshold + 0.07);
    if (tldSuspicious.includes(candidateTld) &&
 ...
 */
export function isSuspicious(similarityScore, threshold, tldPasses) {
    return similarityScore >= threshold && tldPasses;
}
/**
 * Full suspiciousness evaluation combining all rules.
 */
export function evaluateSuspiciousness(candidateTld, watchTld, strategy, allowlist, tldSuspicious, similarityScore, threshold) {
    const tldPasses = evaluateTldStrategy(candidateTld, watchTld, strategy, allowlist);
    const suspicious = isSuspicious(similarityScore, threshold, tldPasses);
    if (!suspicious) {
        return { isSuspicious: false, severity: "INFO" };
    }
    const severity = computeSeverity(candidateTld, watchTld, tldSuspicious, similarityScore, threshold);
