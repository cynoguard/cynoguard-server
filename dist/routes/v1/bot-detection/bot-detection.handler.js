...
import {} from './../../../types/bot-detection.js';
export const verifyHuman = async (request, reply) => {
    ...
    try {
        ...
        if (actionStatus === "challenge" || actionStatus === "uncertain") {
            ...
            const token = jwt.sign({ did: detectionId, cid: challengeData?.id, answer: valueArr[randomPosition - 1] }, process.env.JWT_SECRET, { expiresIn: "5m" });
            ...
            response.data.cookies = { token: jwt.sign({ assessment: "passed" }, process.env.JWT_SECRET, { expiresIn: "3d" }) };
        }
...
export const verifyBotChallenge = async (request, reply) => {
    ...
    try {
        ...
        return reply.code(200).send({ status: "success", message: "Challenge verified", data: { challenge_verified: true, cookies: { token: jwt.sign({ assessment: "passed" }, process.env.JWT_SECRET, { expiresIn: "3d" }) } } });
    }
    catch (error) {
        return reply.code(500).send({ status: "Internal Server Error", message: "", error: error });
    }
...
export const reTakeBotChallenge = async (request, reply) => {
    ...
    try {
        ...
        const decodedToken = jwt.verify(token, process.env.JWT_SECRET);
        const challengeToken = jwt.sign({ cid: challengeData?.id, answer: valueArr[randomPosition - 1] }, process.env.JWT_SECRET, { expiresIn: "5m" });
