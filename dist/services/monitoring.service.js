...
function getThreshold(watchDomainThreshold) {
...
export async function runMonitoringCycle() {
    ...
    for (const wd of watchDomains) {
        try {
            ...
            console.error(`[Monitoring] Error processing watch domain ${wd.domain}:`, error);
        }
    }
    console.log("[Monitoring] Monitoring cycle complete.");
}
async function processWatchDomain(wd) {
    const candidates = await prisma.candidateDomain.findMany({
        where: { watchDomainId: wd.id, isActive: true },
    });
    console.log(`[Monitoring] Processing ${candidates.length} candidates for ${wd.domain}`);
    ...
    for (const candidate of candidates) {
        try {
            ...
            if (rdapResult.registered !== true)
                continue;
            const result = evaluateSuspiciousness(candidate.tld, watchTld, wd.tldStrategy, wd.tldAllowlist, wd.tldSuspicious, candidate.similarityScore, threshold);
