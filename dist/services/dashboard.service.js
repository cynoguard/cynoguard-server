...
export const syncOrganizationData = async (authId) => {
...
export const createNewApiKey = async (name, hashedKey, projectId) => {
...
export const syncApiKeyData = async (id) => {
...
export const syncApiKeyConnectionStatus = async (id) => {
...
export const syncApiKeysList = async (projectId) => {
...
export const updateApiKeyInfo = async (id, status) => {
...
// ─── Org-level overview ───────────────────────────────────────────────────────
export async function getOrgOverview(prisma, organizationId) {
    ...
    const [totalDetections, totalBots, totalMentions, highRiskMentions, totalApiKeys, detectionsPerProject, mentionsPerProject,] = await Promise.all([
...
export async function getProjectOverview(prisma, projectId) {
    const now = Date.now();
    const last7d = new Date(now - 7 * 24 * 60 * 60 * 1000);
    const last24h = new Date(now - 24 * 60 * 60 * 1000);
    const [totalDetections, humanDetections, botDetections, detections24h, totalMentions, newMentions, highRiskMentions, totalApiKeys, activeApiKeys, recentDetections, mentionsByRisk, detectionsByDay,] = await Promise.all([
    ...
    for (const d of detectionsByDay) {
        ...
        if (!dailyMap.has(day))
            dailyMap.set(day, { bots: 0, humans: 0 });
