 ...
 */
export async function listCandidates(watchDomainId, tenantId) {
 ...
 */
export async function addManualCandidates(watchDomainId, tenantId, domains) {
    // Verify watch domain belongs to tenant
    const watchDomain = await prisma.watchDomain.findFirst({
        where: { id: watchDomainId, tenantId },
    });
    ...
    for (const rawDomain of domains) {
        try {
            ...
            if (normalized === watchDomain.domain)
                continue;
            const tld = getTld(normalized);
            const similarityScore = computeSimilarityScore(watchDomain.domain, normalized);
