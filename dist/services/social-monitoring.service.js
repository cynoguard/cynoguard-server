...
function getXClient() {
    ...
    if (!token)
        throw new Error("X_BEARER_TOKEN is not set. Add it to your .env file.");
    _xClient = new TwitterApi(token).readOnly;
    return _xClient;
}
async function fetchTweets(keywords, maxResults = 50) {
...
export async function scanProject(prisma, projectId, logger) {
...
export async function scanAllProjects(prisma, logger) {
    ...
    for (const { projectId } of rows) {
        await scanProject(prisma, projectId, logger).catch((err) => logger.error({ err, projectId }, "[SocialMonitoring] Project scan failed"));
    }
}
